...
def collect_row(run_id, optimizer):
    ...
    row = {
        ...
        "anti_collapse_boost": float(getattr(optimizer, "current_anti_collapse_boost", 0.0)),
        "collapse_risk": float(getattr(optimizer, "current_collapse_risk", 0.0)),
        "guard_strength": float(getattr(optimizer, "current_guard_strength", 0.0)),
...
def write_variable_notes(path):
    notes = [
        ...
        "conv_a_base: Delayed progress-prior base before actor residual and anti-collapse boost.",
        "conv_a_delta: Actor residual contribution.",
        "stagnation_boost: Legacy field; V1 anti-collapse RLCCPSO keeps this at 0.",
        "anti_collapse_boost: Extra Conv_a gated by action[1] and collapse risk.",
        "collapse_risk: V1 Q-collapse risk gate in [0, 1].",
        "guard_strength: max(0, action[1]) used to permit anti-collapse boost.",
